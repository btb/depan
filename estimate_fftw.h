...
// filter parameters
	...
	int improve;	// improve zoom estimation by iteration
...
void frame_data2d (const BYTE * srcp0, int height, int src_width, int pitch, float * fftdata, int winx, int winy, int winleft, bool isYUY2); // v.1.1
...
fftwf_complex * get_plane_fft(const BYTE * srcp,  int src_height, int src_width, int src_pitch, int nsrc, int *fftcachelist, int fftcachecapacity, fftwf_complex **fftcache, int winx, int winy, int winleft, fftwf_plan plan, bool isYUY2); // v.1.1
void showcorrelation(float *realcorrel, int winx, int winy, BYTE *dstp, int dst_pitch, int winleft, int wintop, int isYUY2);
...
public:
  ...
  // Otherwise they can only be called from functions within the class itself.
	...
	DePanEstimate_fftw(PClip _child, int _range, float _trust, int _winx, int _winy, int _dxmax, int _dymax, float _zoommax, int _improve, float _stab, float _pixaspect, int _info, const char * _logfilename, int _debug, int _show, const char * _extlogfilename, IScriptEnvironment* env);
